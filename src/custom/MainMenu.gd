...
#	settings_menu.position.x = get_viewport().size.x
	deck_builder.position.x = -get_viewport().size.x
	card_library.position.x = -get_viewport().size.x
#	settings_menu.back_button.connect("pressed",Callable(self,"_on_Setings_Back_pressed"))
#	settings_menu.recover_prebuilts.connect("pressed",Callable(self,"_on_PreBuilts_pressed"))
