...
func initiate_selection(
	...
	# And we're looking for max or equal amount of cards, we return canceled.
	...
	# We immediately return what is there.
		...
		emit_signal("confirmed")
		return
	# When we have 0 cards to select from, we consider the selection canceled
	...
	var popup_size_x = (card_size.x * thumbnail_scale * shown_columns * cfc.curr_scale)\
			+ _card_grid.get("custom_constants/v_separation") * shown_columns
