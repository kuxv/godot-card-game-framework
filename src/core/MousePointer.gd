...
func determine_global_mouse_pos() -> Vector2:
	...
	#var scaling_offset = get_tree().get_root().get_node('Main').get_viewport().get_size_2d_override() * get_window().size
...
#		if area.get_parent() as BoardPlacementSlot and cfc.card_drag_ongoing and cfc.card_drag_ongoing.is_attachment and not potential_cards.is_empty():
