...
class TestSingleCardFocus:
	...
	func test_single_card_focus_use_rectangle():
		...
		await yield_to(cards[0].get_node('Tween'), "loop_finished", 1).YIELD
...
class TestHandCardSlide:
	...
	func test_card_hand_mouseslide():
		...
		await yield_for(YIELD_TIME).YIELD
		#cards[4]._on_Card_mouse_entered()
		await yield_to(cards[4].get_node('Tween'), "loop_finished", 1).YIELD
