...
func drag_card(card: Card, target_position: Vector2, interpolation_speed := "fast") -> void:
	...
	await yield_for(mouse_yield_wait).YIELD
	click_card(card)
	if interpolation_speed == "debug":
		await yield_for(4).YIELD # Allow for review
	else:
		await yield_for(0.3).YIELD # Wait to allow dragging to start
...
func execute_with_yield(card: Card) -> void:
	...
	if sceng is GDScriptFunctionState and sceng.is_valid():
		sceng = await yield_to(sceng, "completed", 1).YIELD
